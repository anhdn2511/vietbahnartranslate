...
class ItemCustomView(context: Context, attrs: AttributeSet? = null) : View(context, attrs){
    ...
    private var mVietnamese: String = "Hello"
    private var mBahnaric : String = "Xin chao"


    private var mIsLiked : Boolean = true
    ...
    fun setData(data: WordModel) {
        mVietnamese = data.vietnamese
        mBahnaric = data.bahnaric
        mIsLiked = data.isLiked
        mStar =
            if(mIsLiked) ResourcesCompat.getDrawable(resources, R.drawable.ic_baseline_star_24, null)
            else ResourcesCompat.getDrawable(resources, R.drawable.ic_baseline_star_outline_24, null)
        invalidate()
    }
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        when(event?.actionMasked) {
            MotionEvent.ACTION_DOWN -> {
                if (checkX && checkY) {
                    setIsLiked(!mIsLiked)
                    // TODO: handle save liked word to Liked List
                }
            }
        }
        return false
